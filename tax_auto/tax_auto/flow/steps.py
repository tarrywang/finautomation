...
def step_2_navigate_to_query(page: Page, ctx: RunContext) -> StepResult:
    ...
    waits.safe_wait_visible(
        page, "step_2_navigate", "page_loaded_anchor", intent="确认查询页加载完成"
    )
...
def step_3_fill_query_form(page: Page, ctx: RunContext) -> StepResult:
    ...
    waits.safe_fill(
        page, "step_3_fill_query", "input_date_from", ctx.date_from, intent="填开票日期起"
    )
    waits.safe_fill(page, "step_3_fill_query", "input_date_to", ctx.date_to, intent="填开票日期止")
    ...
    if waits.is_visible(page, "step_3_fill_query", "empty_state"):
        ...
        return StepResult(ok=True, next_state=RunState.DONE, artifacts={"expected_count": 0})
    ...
    return StepResult(
        ok=True, next_state=RunState.DOWNLOADING, artifacts={"expected_count": ctx.expected_count}
    )
...
def step_4_select_and_export(page: Page, ctx: RunContext) -> StepResult:
    ...
    waits.safe_click(page, "step_4_select_export", "checkbox_select_all", intent="全选当前页发票")
    waits.safe_click(page, "step_4_select_export", "button_batch_download", intent="点击批量下载")
    waits.wait_for_idle(page)
    # Default OFD per ADR-0006 — fall back to PDF if OFD missing
    try:
        waits.safe_click(page, "step_4_select_export", "format_ofd_radio", intent="选 OFD 格式")
    except SelectorMissError:
        log.warning("OFD radio missing → falling back to PDF")
        waits.safe_click(page, "step_4_select_export", "format_pdf_radio", intent="选 PDF 格式")
    waits.safe_click(
        page, "step_4_select_export", "button_confirm_export", intent="确认提交导出任务"
    )
    waits.safe_wait_visible(
        page, "step_4_select_export", "toast_submitted", intent="等任务提交成功 toast"
    )
...
def step_6_poll_export_zip(page: Page, ctx: RunContext) -> StepResult:
    ...
    while True:
        if waits.is_visible(page, "step_6_poll_export", "status_done", timeout_ms=1500):
            ...
            link = waits.safe_wait_visible(
                page, "step_6_poll_export", "link_download_zip", intent="拿到下载链接"
            )
        ...
        if elapsed_s * 1000 > deadline_ms:
            raise ExportTimeout(f"ZIP not ready within {settings.timeout_export_poll_s}s")
